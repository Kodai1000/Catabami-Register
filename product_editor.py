...
CATEGORY_LABELS = {0: "スイーツ", 1: "ドリンク", 2: "その他"}
...
class ProductEditorApp:
    ...
    def refresh_listbox(self):
        ...
        for product in self.products:
            cat = CATEGORY_LABELS.get(product["category"], "不明")
            self.listbox.insert(tk.END, f'{product["name"]} - ¥{product["price"]} [{cat}]')
    ...
    def open_product_form(self, title, on_submit, product=None):
        ...
        category_menu = tk.OptionMenu(form, category_var, *CATEGORY_LABELS.keys())
